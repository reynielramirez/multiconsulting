...
final class RecipeActivator implements InstructionsInterface, EventSubscriberInterface, TasksInterface {
  ...
  public function activate(Project ...$projects): ?array {
    ...
    // If any of the recipes have inputs, redirect to the form, if we are on a
    // version of Drupal new enough to support it.
    $has_input = fn (Recipe $recipe): bool => (bool) $recipe->input->getDataDefinitions();
    // We need to check for  the trait because it didn't exist in Drupal <11.1.
    // @todo Remove trait_exists() check in https://www.drupal.org/i/3494848.
    if (trait_exists(RecipeInputFormTrait::class) && array_any($recipes, $has_input)) {
