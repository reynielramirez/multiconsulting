...
final class ProjectBrowserInstallerUiTest extends WebDriverTestBase {
  ...
  /**
   * The install progress tracker service.
   */
  private InstallProgress $installProgress;
  ...
  protected function setUp(): void {
    ...
    /** @var \Drupal\project_browser\InstallProgress $install_progress */
    $install_progress = $this->container->get(InstallProgress::class);
    $this->installProgress = $install_progress;
  ...
  public function testApplyRecipe(): void {
    ...
    $card->pressButton('Install');
    // The input form should open in a modal.
  ...
  public function testApplyMultipleRecipes(): void {
    ...
    $this->drupalGet('admin/modules/browse/recipes');
    // Select two recipes that require input.
    $this->searchFor('contact');
